...
static int custom_evaluate(State* state) {
    ...
    for (int r = 0; r < BOARD_H; ++r) {
        for (int c = 0; c < BOARD_W; ++c) {
            if (state->board.board[p][r][c] == 6) { K_r[p] = r; K_c[p] = c; }
            if (state->board.board[opp][r][c] == 6) { K_r[opp] = r; K_c[opp] = c; }
        }
    }

    // 2. 確保 legal_actions 有被生成，才不會算漏 V4, V6
    if (state->legal_actions.empty() && state->game_state == UNKNOWN) {
        state->get_legal_actions();
    }

    // 3. V6: 己方行動力
    score += state->legal_actions.size() * u6_mobility;

    // 4. V4: 戰術威脅 (掃描吃子與將軍)
    for (const auto& action : state->legal_actions) {
        int tr = action.second.first % BOARD_H;
        int tc = action.second.second;
        int target = state->board.board[opp][tr][tc];

        if (target == 6) score += u4_tactical[0]; // 將軍
        else if (target >= 1 && target <= 5) score += u4_tactical[target]; // 威脅吃子
    }

    // 5. 確保步數不會是垃圾值導致整數溢位崩潰
    int safe_step = state->step;
    if (safe_step < 0 || safe_step > 300) safe_step = 20;

    // 6. 掃描棋盤計算 V2, V3, V5, V7
    for (int r = 0; r < BOARD_H; ++r) {
        for (int c = 0; c < BOARD_W; ++c) {
            int my_piece = state->board.board[p][r][c];
            ...
            // --- 處理我方優勢 ---
            if (my_piece > 0) {
                score += u3[my_piece][idx]; // V3
                
                if (K_r[opp] != -1 && my_piece >= 1 && my_piece <= 5) {
                    int dist = std::abs(r - K_r[opp]) + std::abs(c - K_c[opp]);
                    score += dist * u2[my_piece - 1]; // V2
                }
                if (my_piece == 1 && pr >= 1 && pr <= 4) {
                    score += u5_pawn_stages[4 - pr]; // V5
                }
                if (my_piece == 6) {
                    score += u7_king_step[idx] * safe_step; // V7
                }
            }

            // --- 處理敵方扣分 ---
            if (opp_piece > 0) {
                score -= u3[opp_piece][idx]; // V3 扣分
                if (opp_piece == 6) {
                    score -= u7_king_step[idx] * safe_step; // V7 扣分
                }
            }
        }
    }
...
static int score_move(State* state, const Move& move, const Move& tt_move, int ply) {
    ...
    // 這只是隨便糊給 MVV-LVA 的
    static const int mvv_values[7] = {0, 100, 300, 300, 500, 900, 10000};
    // 吃子步：價值 = 基礎高分 + (受害者價值*10) - 攻擊者價值，因為我要盡可能殺人
    if (victim != 0) {
        return 1000000 + (mvv_values[victim] * 10) - mvv_values[attacker];
    }
